...
from checkWin_complete import checkWinPos
...
def evaluate(boardState, depth):
    score = 0
    win = checkWin2(boardState)
    if win == 2:
        score = 100 - depth # AI wins
    if win == 0:
        score = 0 # Tie
    if win == 1:
        score = -100 - depth # AI loses
    # print("score: ", score)
    return score
...
def minimax(depth, maxPlayer, boardState, alpha, beta): 

    # print("depth: ", depth)
    # print("Minimax Board: ", boardState)
    # leaf node is reached  
    nextMoves = genMoves(boardState)

    # Terminating condition. i.e 
    if checkWin2(boardState)!= None or not nextMoves:  
        score = evaluate(boardState, depth)
        return score
    ...
    if maxPlayer:  
        ...
        for i in nextMoves:
            boardState[i] = 2 #set a board pos
            val = minimax(depth+1, False, boardState, alpha, beta)
            boardState[i] = 0 #reset the board pos
            # print("Val MAX: ", val)
            best = max(best, val)
            # print("Best MAX: ", best)
        ...
        # Recur for left and right children
        for i in nextMoves:
            boardState[i] = 1  #set a board pos
            val = minimax(depth+1, True, boardState, alpha, beta)
            boardState[i] = 0  #reset the board pos
            # print("Val MIN: ", val)  
            best = min(best, val)
            # print("Best MIN: ", best)
            beta = min(beta, best) 
            # print("beta MIN: ", alpha)
            # Alpha Beta Pruning  
            if beta <= alpha: 
                # print("MIN break") 
                break
...
def decisionMaker(board):

    # print("DecisionMaker on board: " , board)
    # AI to make its turn
    bestScore = -np.inf
    move = 10
    nextMoves = genMoves(board)
    
    for i in nextMoves:
        board[i] = 2  #set a board pos
        score = minimax(0, False, board, MIN, MAX)
        board[i] = 0  #reset the board pos
        if (score > bestScore): #check if its the best score
            bestScore = score
            move = i
    return move
